...
{
    public partial class nivelTres : Form
    {
        ...
        List<PictureBox> bolasFuego = new List<PictureBox>();
        int contadorDisparoBoss = 0;
        int intervaloDisparoBoss = 40;
        ...
        public nivelTres()
        {
            ...
            timerDisparo.Interval = 80;
            timerDisparo.Tick += (s, e) =>
            {
                if (balas > 0 && !gameOver)
                {
                    balas--;
        ...
        private void MainTimerEvent(object sender, EventArgs e)
        {
            ...
            if (boss != null)
            {
                ...
                if (contadorDisparoBoss >= intervaloDisparoBoss)
                {
                    contadorDisparoBoss = 0;
                    DispararFuego();
                }
            }
                ...
                for (int j = this.Controls.Count - 1; j >= 0; j--)
                {
                    ...
                    if (c is PictureBox b && b.Tag != null && b.Tag.ToString() == "bala")
                    {
                        if (z.Bounds.IntersectsWith(b.Bounds))
                        {
                            ...
                            if (vidaZombies.ContainsKey(z) && vidaZombies[z] <= 0)
                            {
                                score++;
        ...
        private void MoverZombie(PictureBox z)
        {
            ...
            if (Math.Abs(dx) > Math.Abs(dy))
            {
                ...
                if (dx > 0)
                    z.Image = Resources.zright1;
                else
                    z.Image = Resources.zleft1;
            }
            else
            {
                ...
                if (dy > 0)
                    z.Image = Resources.zdown1;
                else
                    z.Image = Resources.zup1;
            }
        }
        ...
        private void KeyIsDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Left) { movIzquierda = true; facing = "left"; player.Image = Resources.left3; }
            if (e.KeyCode == Keys.Right) { movDerecha = true; facing = "right"; player.Image = Resources.right3; }
            if (e.KeyCode == Keys.Up) { movArriba = true; facing = "up"; player.Image = Resources.up3; }
            if (e.KeyCode == Keys.Down) { movAbajo = true; facing = "down"; player.Image = Resources.down3; }
        ...
        {
            player.Image = Resources.up3;
            velocidadBase = velocidad;

            speedBoostActivo = false;
            powerupActivo = false;

            foreach (PictureBox fuego in bolasFuego)
            {
                Controls.Remove(fuego);
                fuego.Dispose();
            }
        ...
        private void MoverBoss()
        {
            ...
            if (Math.Abs(dx) > Math.Abs(dy))
            {
                ...
                if (dx > 0)
                    boss.Image = Resources.bossright;
                else
                    boss.Image = Resources.bossleft;
            }
            else
            {
        ...
        {
            PictureBox fuego = new PictureBox();

            fuego.Size = new Size(35, 35);
            fuego.Image = Resources.fuego;
            fuego.SizeMode = PictureBoxSizeMode.StretchImage;
            fuego.BackColor = Color.Transparent;

            fuego.Left = boss.Left + boss.Width / 2;
            fuego.Top = boss.Top + boss.Height / 2;

            fuego.Tag = new Point(
                player.Left - boss.Left,
                player.Top - boss.Top
            );

            bolasFuego.Add(fuego);
            Controls.Add(fuego);
            fuego.BringToFront();
        }

        private void MoverBolasFuego()
        {
            for (int i = bolasFuego.Count - 1; i >= 0; i--)
            {
                PictureBox fuego = bolasFuego[i];

                Point dir = (Point)fuego.Tag;

                double distancia = Math.Sqrt(dir.X * dir.X + dir.Y * dir.Y);

                if (distancia == 0) distancia = 1;

                fuego.Left += (int)(dir.X / distancia * 12);
                fuego.Top += (int)(dir.Y / distancia * 12);

                if (fuego.Bounds.IntersectsWith(player.Bounds))
                {
                    vidaJugador -= 10;

                    Controls.Remove(fuego);
                    fuego.Dispose();
                    bolasFuego.RemoveAt(i);
                    continue;
                }

                if (fuego.Right < 0 ||
                    fuego.Left > ClientSize.Width ||
                    fuego.Bottom < 0 ||
                    fuego.Top > ClientSize.Height)
                {
                    Controls.Remove(fuego);
                    fuego.Dispose();
                    bolasFuego.RemoveAt(i);
        ...
        private void nivelTres_Load(object sender, EventArgs e)
        {
